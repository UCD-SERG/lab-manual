...
When embedding raw source code from external files without executing them,
the [`include-code-files`](https://github.com/quarto-ext/include-code-files) extension provides a shortcode
to transclude code files with automatic syntax highlighting,
line numbers,
and line-range selection:

```markdown
{{</* include-code-files path/to/script.R */>}}
```

**Key capabilities:**

- Embed external scripts directly without copy-pasting
- Slice specific line ranges with parameters (e.g., `snippet="main-function"`, `start-line=10`, `end-line=25`)
