...
Adapted by UCD-SeRG team from [original by Anna Nguyen, Jade Benjamin-Chung, and Gabby Barratt Heitmann](https://jadebc.github.io/lab-manual/data-masking.html)
...
- **Data-masking** contexts treat column names as values you compute *with*:
  ...
  `group_by()`, `arrange()`, and `aes()` in [`{ggplot2}`](https://ggplot2.tidyverse.org/).
- **Tidy-selection** contexts treat column names as columns you *choose*:
  ...
  and the `cols`, `names_from`, `values_from`, and `id_cols` arguments of `tidyr::pivot_*()`.
...
Using it in a selection context was deprecated in [`{tidyselect}`](https://tidyselect.r-lib.org/) 1.2.0
...
a soft-deprecated feature warns only users who trigger it from the global environment
and developers who use it directly (for example when running [`{testthat}`](https://testthat.r-lib.org/) tests);
there is *no warning when the deprecated feature is called indirectly by another package*.
...
because `warn = 2` only converts warnings into errors once a warning is actually signaled,
and for a soft deprecation in installed-package code no warning is signaled in the first place.
