...
- **Data-masking** contexts treat column names as values you compute *with*: `mutate()`, `filter()`, `summarize()`/`summarise()` (the summary expressions), `group_by()`, `arrange()`, and `aes()` in `ggplot2`.
...
The `.data` pronoun is a *data-masking* tool, not a selection tool. Using it in a selection context was deprecated in `tidyselect` 1.2.0, but it is a **soft** deprecation: in installed-package code it usually emits *no warning at all*, so it slips past interactive use, CI, and code review unnoticed. Rewrite these with `all_of()`/`any_of()` and plain strings:
