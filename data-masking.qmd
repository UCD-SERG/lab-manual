...
Many [`{tidyverse}`](https://www.tidyverse.org/) verbs use *two* different flavors of tidy evaluation, and the way you refer to a column stored in a string variable depends on which one you are in.

- **Data-masking** contexts treat column names as values you compute *with*: `mutate()`, `filter()`, `summarize()`/`summarise()` (the summary expressions), `group_by()`, `arrange()`, and `aes()` in [`{ggplot2}`](https://ggplot2.tidyverse.org/).
- **Tidy-selection** contexts treat column names as columns you *choose*: `select()`, `rename()`, `relocate()`, `pull()`, the `.cols` argument of `across()`, the `.by` argument of `mutate()`/`summarize()`, and the `cols`, `names_from`, `values_from`, and `id_cols` arguments of `tidyr::pivot_*()`.
...
The `.data` pronoun is a *data-masking* tool, not a selection tool. Using it in a selection context was deprecated in [`{tidyselect}`](https://tidyselect.r-lib.org/) 1.2.0, but it is a **soft** deprecation: in installed-package code it usually emits *no warning at all*, so it slips past interactive use, CI, and code review unnoticed. Rewrite these with `all_of()`/`any_of()` and plain strings:
...
To make this class of soft deprecation fail loudly, set `options(lifecycle_verbosity = "error")` in your tests (for example in `tests/testthat/setup.R`). Plain `options(warn = 2)` will *not* catch it, because the warning is suppressed before it is ever signaled.
