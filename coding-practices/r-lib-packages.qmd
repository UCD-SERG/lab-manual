...
It allows regular expressions to be composed modularly using functions such as `rex()`, `character_class()`, `capture()`, and `regex_group()`.

When using `{rex}` inside another R package:

- Add `rex` to `Imports` in the package `DESCRIPTION`.
- Call `rex::rex()` or selectively import specific helper verbs into the package namespace to avoid collisions.
- Use `rex::rex(..., regex)` to compile expressions into standard regular expression strings for use with functions like `grepl()`, `gsub()`, or `stringr` functions.
