...
and provides `memoise::forget()` and `memoise::is.memoised()` to manage and inspect cache state.
...
Do not memoise stochastic simulation routines or pseudorandom number generators
unless the random seed or iteration index is explicitly included in the formal arguments,
as memoisation will otherwise suppress stochastic variability and return identical draws.
