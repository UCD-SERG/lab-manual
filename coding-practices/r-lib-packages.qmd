...
[`{memoise}`](https://memoise.r-lib.org/) adds memoisation
(function-call caching) to R functions.
Wrapping an expensive, pure function with `memoise::memoise()`
causes it to cache computed return values in memory or on disk,
so repeated calls with identical arguments return instantly from cache
...
and `memoise::has_cache()` to check whether specific arguments
have cached results.
...
Never memoise stochastic simulation routines
or functions that generate pseudorandom numbers;
memoisation suppresses stochastic variation across repeated calls,
silently collapsing random variability by returning stale cached draws.
