...
Wrapping an expensive or pure function with `memoise()` causes it to cache computed return values
...
(including `cachem::cache_mem()` for in-memory caching and `cachem::cache_disk()` for persistent disk caching across sessions),
and provides `forget()` and `is.memoised()` to manage and inspect cache state.
Use `{memoise}` to avoid redundant computation in iterative simulations,
API queries,
or repeated data extraction steps
where underlying inputs remain unchanged.
