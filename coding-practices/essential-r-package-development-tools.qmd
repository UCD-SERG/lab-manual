...
Running `R CMD check` workflows on GitHub Actions introduces runner startup overhead and delay,
as virtual environments must be provisioned and system-level dependencies configured on each run.
Always run `devtools::check()` locally before pushing to GitHub
to catch errors, warnings, and NOTEs early while conserving CI resources.
:::
