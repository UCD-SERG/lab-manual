...
│   └── private/        # Private analyses (see below)
...
#### Private Analyses (inst/private/)

For analyses that rely on **private, sensitive, or restricted data**, place `.qmd` or `.Rmd` files in `inst/private/`:
...
- Private analyses are included in version control alongside your code
- They're clearly separated from public documentation
- `inst/private/` is **excluded from `pkgdown` builds** and package documentation
- Collaborators with data access can still run these analyses
- You maintain a complete record of all project work

**Best practices for private analyses:**

1. **Document data requirements**: Include a README.md in `inst/private/` explaining:
...
2. **Use relative paths carefully**: Structure your code so data paths can be configured:
   ```r
   # In inst/private/01-analysis.qmd
   # Users should set this based on their local setup
   data_dir <- Sys.getenv("MYPROJECT_PRIVATE_DATA", 
                          default = "~/private_data/myproject")
...
4. **Add to .Rbuildignore**: Ensure `inst/private/` doesn't cause package checks to fail:
   ```
   # In .Rbuildignore
   ^inst/private$
   ```
...
4. **In `inst/private/`**: Put analyses using private/restricted data
...
**File headers** (for scripts in `data-raw/` or `inst/private/`):
...
saveRDS(results, here("inst", "private", "results.rds"))
```
