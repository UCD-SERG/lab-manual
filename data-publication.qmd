# Data Publication {#sec-data-publication}
...
For learning about working with geospatial data, see the [UC Davis DataLab GIS workshops](https://github.com/ucdavisdatalab), including resources on QGIS and spatial SQL. 
...
Please see the reproducible example provided by the
[Proctor Lab at UCSF](https://proctor-ucsf.github.io/dcc-handbook/publicdata.html)
that you can run and replicate when making data public for your projects.
The example contains the following files and folders:
...
First, ensure that you create a codebook and metadata file for each public dataset
([see the DCC guide on Documenting datasets](https://proctor-ucsf.github.io/dcc-handbook/datawrangling.html#documenting-datasets)).
Use the same name as the datasets,
but with `-codebook.txt`, `-codebook.html`, or `-codebook.csv` at the end
(depending on the file format for the codebook).
One option is the R `codebook` package,
which also generates JSON output that is machine-readable.

For modern, standardized data dictionaries,
consider the [Tidyverse data-dict](https://data-dict.tidyverse.org/) project.
The `data-dict.yaml` specification defines an open,
lightweight,
plain-text YAML format for describing tabular datasets---including table schemas,
column data types,
descriptions,
relationships,
and validation constraints.
Because data dictionaries are defined in plain-text YAML files,
they can be version-controlled directly in Git alongside your analysis code.
The companion `data-dict` command-line tool can draft dictionaries from existing data files,
validate dataset values against your dictionary rules,
and render self-contained HTML documentation reports.

For additional guidance on data documentation best practices,
see the [UC Davis DataLab workshop on data documentation](https://github.com/ucdavisdatalab/workshop_how-to-data-documentation).  
...
5. Create a Digital Object Identifier (DOI) for the repository. A DOI is a unique identifier that provides a persistent link to content, such as a dataset in this case. [Learn more about DOIs](https://researchdata.princeton.edu/research-lifecycle-guide/publishing-and-preservation/dois)
...
Further reading on end-to-end data management: How to Store and Manage Your Data - PLOS [@plos_data]
