...
If your R package is intended for public distribution, you may want to release it on [CRAN](https://cran.r-project.org/) (the Comprehensive R Archive Network). This section summarizes the release process and adds lab-specific guidance on versioning and tagging. For full details, see the [R Packages book chapter on releasing to CRAN](https://r-pkgs.org/release.html) by Hadley Wickham and Jennifer Bryan.

The diagram below illustrates the stages of the CRAN submission pipeline:

![Stages of the CRAN submission pipeline. Source: [Edgar Ruiz, cran-stages](https://github.com/edgararuiz-zz/cran-stages).](https://raw.githubusercontent.com/edgararuiz-zz/cran-stages/master/cran-stages.png){fig-alt="A flowchart showing the stages that an R package goes through during CRAN submission, from initial submission through automated checks, human review, and final acceptance or rejection."}
...
2. **Run `devtools::check()`**: Ensure your package passes `R CMD CHECK` with no errors, warnings, or notes. Address any issues before submitting.
   ```r
   devtools::check()
   ```
...
4. **Test on multiple platforms**: CRAN requires packages to work across operating systems and R versions. Use the following to test on additional platforms:
   - `devtools::check_win_devel()` — tests on Windows with the development version of R
   - `rhub::check_for_cran()` — tests on multiple platforms via [R-hub](https://r-hub.github.io/rhub/)

5. **Review CRAN policies**: Ensure your package complies with the [CRAN Repository Policy](https://cran.r-project.org/web/packages/policies.html).
...
This bundles your package and uploads it to CRAN's submission portal. You will receive an email asking you to confirm the submission. CRAN maintainers typically review submissions within a few days to a few weeks.

In your submission comments (prompted by `devtools::submit_cran()`), briefly describe any `R CMD CHECK` notes and explain why they are acceptable. Be concise and factual.

For first-time submissions, also check the [new submission checklist](https://r-pkgs.org/release.html#sec-release-initial) in the R Packages book for additional requirements.

### After CRAN Acceptance

Once CRAN accepts your package:

1. **Tag the release in git**: Create an annotated git tag for the released version. Use the convention `v` followed by the version number (e.g., `v1.2.0`):
   ```bash
...
2. **Create a GitHub release**: On GitHub, navigate to **Releases → Draft a new release**, select the tag you just pushed, and publish a release. Copy the relevant section of `NEWS.md` as the release notes. GitHub releases make it easy for users to download source archives and review the changelog.

3. **Bump the development version**: After releasing, increment the patch version and add the `.9000` development suffix in `DESCRIPTION`:
   ```r
...
4. **Commit the version bump**: Stage and commit the updated `DESCRIPTION` (and `NEWS.md` if you added a `# mypackage (development version)` header):
   ```bash
