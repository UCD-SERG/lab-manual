...
A `README.md` should live at the top directory of the project.
Include a repository status badge
from [repostatus.org](https://www.repostatus.org/)
near the top of the file
to communicate whether the codebase is actively maintained,
a work in progress,
or an archived analysis (see @sec-repostatus).
This file also includes a Project Overview and a Directory Structure,
along with contributor names and the project license.
...
Detailed instructions: [Managing releases in a repository](https://docs.github.com/en/repositories/releasing-projects-on-github/managing-releases-in-a-repository).
...
For full details,
see the [R Packages book chapter on releasing to CRAN](https://r-pkgs.org/release.html)
...
5. **Test on multiple platforms**: CRAN requires packages to work across operating systems and R versions.
   ...
   - [R-hub](https://github.com/r-hub/rhub) - tests on a wide range of platforms.
     In [rhub v2](https://r-hub.github.io/rhub/articles/rhubv2.html), this runs via GitHub Actions:
     run `rhub::rhub_setup()` once to add the workflow to your repository,
     then `rhub::rhub_check()` to trigger a check run.
      (If you previously used older rhub APIs, see the rhub v2 documentation for migration details.)
...
Review time varies; watch for email from CRAN maintainers with acceptance or requested changes.
...
1. **Tag the release and create a GitHub release**:
   ...
   using a tag named `v` followed by the version number (e.g., `v1.2.0`).
