...
5. **Test on multiple platforms**: CRAN requires packages to work across operating systems and R versions. Use the following to test on additional platforms:
   ...
   - [R-hub](https://github.com/r-hub/rhub) - tests on a wide range of platforms. As of rhub v2 this runs via GitHub Actions: run `rhub::rhub_setup()` once to add the workflow to your repository, then `rhub::rhub_check()` to trigger a check run. (The older `rhub::check_for_cran()` function was removed in the v2 rewrite.)

6. **Review CRAN policies**: Ensure your package complies with the [CRAN Repository Policy](https://cran.r-project.org/web/packages/policies.html).
...
1. **Tag the release and create a GitHub release**:
   Follow the steps in the [Create Github release](#create-github-release) section above,
...
2. **Bump the development version**: After releasing, increment the patch version and add the `.9000` development suffix in `DESCRIPTION`:
   ```r
